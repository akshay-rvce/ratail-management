  ...
  <style>
		...
		<body>
				...
				<?php
								...
								echo "<center><h1>$stype Report of $bname</h1></center>";
				...
				<table>
						...
						<?php
								...
								if($stype=="sales")
								{
									...
									$res=mysql_query("SELECT o.inv_no, `orderid`, `item_id`, `item_name`, `base_price`, sell_price,`quantity`, `amount`,s.date,s.time FROM `order` o,sales s WHERE o.inv_no=s.inv_no and s.manager_email_id='$bemail'") or die("Error".mysql_error());
									while($row=mysql_fetch_row($res))
								...
								{
									echo "$bemail";
									 echo "<tr><th>item_id</th><th>item_name</th><th>Base price</th><th>Sell price</th><th>quantity</th></tr>";
									$res=mysql_query("SELECT distinct `item_id`, `item_name`, `base_price`, sell_price,`quantity` FROM stock s,branch b WHERE s.manager_email_id='$bemail'") or die("Error".mysql_error());
									while($row=mysql_fetch_row($res))
